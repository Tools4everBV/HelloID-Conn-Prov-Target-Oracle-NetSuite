#####################################################
# HelloID-Conn-Prov-Target-Oracle-Netsuite-Disable
#
# Version: 1.0.0
#####################################################
...
try {
    ...
    try {
        ...
        $action = 'Found'
        $dryRunMessage = "Disable Oracle-Netsuite Employee account for: [$($p.DisplayName)] will be executed during enforcement"
    } catch {
    ...
    if (-not($dryRun -eq $true)) {
        switch ($action) {
            'Found' {
                Write-Verbose "Disable Oracle-Netsuite Employee account with accountReference: [$aRef]"
                $requestUri = "$($config.BaseUrl)/services/rest/record/v1/employee/$aRef"
                $Method = 'PATCH'
                ...
                $splatParams = @{
                    ...
                    Body        =  @{isInactive = $true } | ConvertTo-Json
                }
                $null = Invoke-RestMethod @splatParams -Verbose:$false #204

                $auditLogs.Add([PSCustomObject]@{
                        Message = 'Disable Oracle-Netsuite Employee account was successful'
                        IsError = $false
                    })
                break
            }
            ...
            'NotFound' {
                $auditLogs.Add([PSCustomObject]@{
                        Message = "Oracle-Netsuite Employee account for: [$($p.DisplayName)] not found. Possibily already deleted. Skipping action"
                        IsError = $false
                    })
                break
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not disable Oracle-Netsuite Employee account. Error: $($errorObj.FriendlyMessage)"
        Write-Verbose "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not disable Oracle-Netsuite Employee account. Error: $($ex.Exception.Message)"
