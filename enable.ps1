#####################################################
# HelloID-Conn-Prov-Target-Oracle-Netsuite-Enable
#
# Version: 1.0.0
#####################################################
...
try {
    ...
    if ($dryRun -eq $true) {
        Write-Warning "[DryRun] Enable Oracle-Netsuite Employee account for: [$($p.DisplayName)] will be executed during enforcement"
    }

    # Process
    if (-not($dryRun -eq $true)) {
        Write-Verbose "Enabling Oracle-Netsuite Employee account with accountReference: [$aRef]"
        ...
        $splatParams = @{
            ...
            Body        = @{isInactive = $false } | ConvertTo-Json
        }
        $null = Invoke-RestMethod -Verbose:$false @splatParams #204

        $success = $true
        $auditLogs.Add([PSCustomObject]@{
                Message = 'Enable Oracle-Netsuite Employee account was successful'
                IsError = $false
            })
    }
} catch {
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not enable Oracle-Netsuite Employee account. Error: $($errorObj.FriendlyMessage)"
        Write-Verbose "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not enable Oracle-Netsuite Employee account. Error: $($ex.Exception.Message)"
